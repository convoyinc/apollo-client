...
describe('QueryManager', function () {
    ...
    var createQueryManager = function (_a) {
        var networkInterface = _a.networkInterface, _b = _a.addTypename, addTypename = _b === void 0 ? false : _b, _c = _a.config, config = _c === void 0 ? {} : _c;
    ...
    var mockMutation = function (_a) {
        var mutation = _a.mutation, data = _a.data, _b = _a.variables, variables = _b === void 0 ? {} : _b, _c = _a.config, config = _c === void 0 ? {} : _c;
    ...
    it('supports interoperability with other Observable implementations like RxJS', function (done) {
        ...
        observable.map(function (result) { return assign({ fromRx: true }, result); }).subscribe({
    ...
    it('allows you to subscribe twice to one query', function (done) {
        ...
        var subOneCount = 0;
        queryManager.query(request).then(function () {
    ...
    it('allows you to refetch queries with promises', function () {
        ...
        return handle
            .refetch()
            .then(function (result) { return assert.deepEqual(result.data, data2); });
        var _a;
    });
    ...
    it('deepFreezes results in development mode', function () {
        ...
        return queryManager.query({ query: query }).then(function (result) {
            assert.deepEqual(result.data, data);
            assert.throws(function () { return (result.data.stuff = 'awful'); });
        });
        var _a;
    ...
    it('runs a mutation with object parameters and puts the result in the store', function () {
        ...
        return mockMutation({
            ...
            assert.deepEqual(result.data, data);
            assert.deepEqual(queryManager.dataStore.getCache().getData()['5'], {
                id: '5',
                isPrivate: true,
            });
        });
    ...
    it("doesn't return data while query is loading", function () {
        ...
        return Promise.all([
            observableToPromise({ observable: observable1 }, function (result) {
                return assert.deepEqual(result.data, data1);
            }),
            observableToPromise({ observable: observable2 }, function (result) {
                return assert.deepEqual(result.data, data2);
            }),
        ]);
        var _a, _b;
    });
    it("updates result of previous query if the result of a new query overlaps", function () {
        ...
        return observableToPromise({ observable: observable }, function (result) {
            ...
            return assert.deepEqual(result.data, {
                people_one: {
                    name: 'Luke Skywalker has a new name',
                    age: 50,
                },
    ...
    describe('polling queries', function () {
        ...
        it('should let you handle multiple polled queries and unsubscribe from one of them', function (done) {
            ...
            var subscription1 = queryManager
                .watchQuery({
                query: query1,
                pollInterval: 150,
            })
            ...
            var subscription2 = queryManager
                .watchQuery({
                query: query2,
                pollInterval: 2000,
            })
    ...
    describe('store resets', function () {
        it('returns a promise resolving when all queries have been refetched', function () {
            ...
            return Promise.all([
                observableToPromise({ observable: observable }, function (result) {
                    return assert.deepEqual(result.data, data);
                }),
                observableToPromise({ observable: observable2 }, function (result) {
                    return assert.deepEqual(result.data, data2);
                }),
        ...
        it('should change the store state to an empty state', function () {
            ...
            assert.deepEqual(queryManager.dataStore.getCache().getData(), {});
    ...
    it('should error when we attempt to give an id beginning with $', function (done) {
        ...
        var reducerConfig = {
            dataIdFromObject: function (x) { return '$' + dataIdFromObject(x); },
        };
        createQueryManager({
            networkInterface: mockNetworkInterface({
                request: { query: query },
    ...
    it('should not empty the store when a non-polling query fails due to a network error', function (done) {
        ...
        queryManager
            ...
            queryManager
                ...
                .catch(function (error) {
                assert.deepEqual(queryManager.dataStore.getCache().getData()['$ROOT_QUERY.author'], data['author']);
                done();
            });
        })
            .catch(function (error) {
    ...
    it('should error if we replace a real id node in the store with a generated id node', function () {
        ...
        var observableWithId = queryManager.watchQuery({
            query: queryWithId,
        });
        var observableWithoutId = queryManager.watchQuery({
            query: queryWithoutId,
        });
        return Promise.all([
            observableToPromise({ observable: observableWithId, wait: 60 }, function (result) {
                return assert.deepEqual(result.data, dataWithId);
            }),
            observableToPromise({
                observable: observableWithoutId,
                errorCallbacks: [function (error) { return assert.include(error.message, 'Store error'); }],
                wait: 60,
            }),
        ]);
        var _a, _b;
    ...
    it('should not error when merging a generated id store node  with a real id node', function () {
        ...
        return Promise.all([
            ...
            observableToPromise({ observable: observableWithId, wait: 120 }, function (result) {
                return assert.deepEqual(result.data, dataWithId);
            }),
        ]);
        var _a, _b;
