...
var ApolloClient_1 = require("../src/ApolloClient");
...
describe('QueryManager', function () {
    ...
    var assertWithObserver = function (_a) {
        ...
        return queryManager.watchQuery(finalOptions).subscribe({
            next: wrap_1.default(done, observer.next),
    ...
    var mockRefetch = function (_a) {
        ...
        return mockQueryManager_1.default({
            request: request,
            result: firstResult,
        }, {
            request: request,
    ...
    it('supports interoperability with other Observable implementations like RxJS', function (done) {
        ...
        var handle = mockWatchQuery_1.default({
            request: {
                query: (_a = ["\n          query people {\n            allPeople(first: 1) {\n              people {\n              name\n            }\n          }\n        }"], _a.raw = ["\n          query people {\n            allPeople(first: 1) {\n              people {\n              name\n            }\n          }\n        }"], graphql_tag_1.default(_a)),
            },
            result: expResult,
        ...
        observable
            ...
            .subscribe({
            next: wrap_1.default(done, function (newResult) {
                var expectedResult = assign({ fromRx: true, loading: false, networkStatus: 7 }, expResult);
    ...
    it('sets networkStatus to `refetch` when refetching', function () {
        ...
        return observableToPromise_1.default({ observable: observable }, function (result) {
            chai_1.assert.deepEqual(result.data, data1);
            observable.refetch();
        }, function (result) { return chai_1.assert.equal(result.networkStatus, store_2.NetworkStatus.refetch); }, function (result) {
            chai_1.assert.equal(result.networkStatus, store_2.NetworkStatus.ready);
            chai_1.assert.deepEqual(result.data, data2);
        });
        var _a;
    });
        ...
        var handle = observable.subscribe({
            next: function (result) {
                ...
                if (counter === 1) {
                    ...
                    chai_1.assert.equal(result.networkStatus, store_2.NetworkStatus.poll);
                    handle.unsubscribe();
                    done();
                }
            },
    ...
    it("doesn't return data while query is loading", function () {
        ...
        return Promise.all([
            observableToPromise_1.default({ observable: observable1 }, function (result) { return chai_1.assert.deepEqual(result.data, data1); }),
            observableToPromise_1.default({ observable: observable2 }, function (result) { return chai_1.assert.deepEqual(result.data, data2); }),
        ]);
        var _a, _b;
    });
    ...
    describe('polling queries', function () {
        it('allows you to poll queries', function () {
            ...
            return observableToPromise_1.default({ observable: observable }, function (result) { return chai_1.assert.deepEqual(result.data, data1); }, function (result) { return chai_1.assert.deepEqual(result.data, data2); });
            var _a;
        });
        ...
        it('allows you to unsubscribe from polled queries', function () {
            ...
            var _b = observableToPromise_1.observableToPromiseAndSubscription({
        ...
        it('exposes a way to stop a polling query', function () {
            ...
            return observableToPromise_1.default({ observable: observable, wait: 60 }, function (result) {
                chai_1.assert.deepEqual(result.data, data1);
                observable.stopPolling();
            });
            var _a;
        ...
        it('stopped polling queries still get updates', function () {
            ...
            var timeout;
            return Promise.race([
                observableToPromise_1.default({ observable: observable }, function (result) {
                    chai_1.assert.deepEqual(result.data, data1);
    ...
    describe('store resets', function () {
        ...
        it('should only refetch once when we store reset', function () {
            ...
            var observable = queryManager.watchQuery({ query: query });
            return observableToPromise_1.default({ observable: observable, wait: 0 }, function (result) { return chai_1.assert.deepEqual(result.data, data); }).then(function () {
        ...
        it('should not refetch toredown queries', function (done) {
            ...
            observableToPromise_1.default({ observable: observable, wait: 0 }, function (result) { return chai_1.assert.deepEqual(result.data, data); }).then(function () {
        ...
        it('should not error on queries that are already in the store', function () {
            ...
            queryManager = createQueryManager({ networkInterface: networkInterface });
            var observable = queryManager.watchQuery({ query: query });
            return observableToPromise_1.default({ observable: observable, wait: 20 }, function (result) { return chai_1.assert.deepEqual(result.data, data); }).then(function () {
                chai_1.assert.equal(timesFired, 2);
            });
            var _a;
        });
    ...
    it('should be able to unsubscribe from a polling query subscription', function () {
        ...
        var observable = mockQueryManager_1.default({
            request: { query: query },
            result: { data: data },
    ...
    it('should not empty the store when a polling query fails due to a network error', function () {
        ...
        return observableToPromise_1.default({
            observable: observable,
            errorCallbacks: [
                function () {
                    chai_1.assert.deepEqual(queryManager.store.getState().apollo.data['$ROOT_QUERY.author'], data.author);
    ...
    it('should not fire next on an observer if there is no change in the result', function () {
        ...
        var observable = queryManager.watchQuery({ query: query });
        return Promise.all([
            observableToPromise_1.default({ observable: observable, wait: 100 }, function (result) {
                chai_1.assert.deepEqual(result.data, data);
            }),
            queryManager.query({ query: query }).then(function (result) {
                chai_1.assert.deepEqual(result.data, data);
    ...
    it('should store metadata with watched queries', function () {
        ...
        return observableToPromise_1.default({ observable: observable }, function (result) {
            chai_1.assert.deepEqual(result.data, data);
            chai_1.assert.deepEqual(queryManager.getApolloState().queries[observable.queryId].metadata, { foo: 'bar' });
        });
        var _a;
    });
    ...
    it('should error if we replace a real id node in the store with a generated id node', function () {
        ...
        return Promise.all([
            observableToPromise_1.default({ observable: observableWithId, wait: 60 }, function (result) { return chai_1.assert.deepEqual(result.data, dataWithId); }),
            observableToPromise_1.default({
    ...
    it('should not error when merging a generated id store node  with a real id node', function () {
        ...
        var observableWithoutId = queryManager.watchQuery({ query: queryWithoutId });
        return Promise.all([
            observableToPromise_1.default({ observable: observableWithoutId, wait: 120 }, function (result) { return chai_1.assert.deepEqual(result.data, dataWithoutId); }, function (result) { return chai_1.assert.deepEqual(result.data, mergedDataWithoutId); }),
            observableToPromise_1.default({ observable: observableWithId, wait: 120 }, function (result) { return chai_1.assert.deepEqual(result.data, dataWithId); }),
        ]);
        var _a, _b;
    });
    ...
    describe('result transformation', function () {
            ...
            client = new ApolloClient_1.default({
                ...
                resultTransformer: function (result) {
                    ...
                    return {
                        ...
                        loading: false,
                        networkStatus: store_2.NetworkStatus.ready,
                    };
                },
            });
        });
        ...
        it('transforms watchQuery() currentResult()', function () {
            response = { data: { foo: 123 } };
            var observable = client.watchQuery({ query: (_a = ["{ foo }"], _a.raw = ["{ foo }"], graphql_tag_1.default(_a)) });
            return observableToPromise_1.default({ observable: observable }, function () {
                var result = observable.currentResult();
                chai_1.assert.isNumber(result.data.transformCount);
            });
            var _a;
        });
        it('does not transform identical watchQuery() results', function () {
            ...
            var succeed;
            return Promise.race([
                observableToPromise_1.default({ observable: observable, shouldResolve: false }, function (result) {
                    chai_1.assert.deepEqual(result.data, { foo: 123, transformCount: 1 });
