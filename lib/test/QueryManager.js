...
describe('QueryManager', function () {
    ...
    it('does not call broadcastNewStore when Apollo state is not affected by an action', function () {
        ...
        var observable = qm.watchQuery({ query: query, variables: variables });
        return observableToPromise({ observable: observable }, function (result) {
            assert.deepEqual(result.data, data1);
            observable.refetch();
        }, function (result) {
    ...
    it('should error when we orphan a real-id node in the store with a real-id node', function () {
        ...
        return Promise.all([
            observableToPromise({
                ...
                errorCallbacks: [
                    function (error) { return assert.include(error.networkError.message, 'find field'); },
                ],
                wait: 60,
            }, function (result) { return assert.deepEqual(result.data, data1); }),
            observableToPromise({
    ...
    describe('loading state', function () {
        ...
        it('will be true when partial data may be returned', function (done) {
            var query1 = (_a = ["{\n        a { x1 y1 z1 }\n      }"], _a.raw = ["{\n        a { x1 y1 z1 }\n      }"], gql(_a));
            var query2 = (_b = ["{\n        a { x1 y1 z1 }\n        b { x2 y2 z2 }\n      }"], _b.raw = ["{\n        a { x1 y1 z1 }\n        b { x2 y2 z2 }\n      }"], gql(_b));
            var data1 = {
                a: { x1: 1, y1: 2, z1: 3 },
            ...
            queryManager.query({ query: query1 }).then(function (result1) {
    ...
    describe('result transformation', function () {
        ...
        it('transforms query() results', function () {
            ...
            return client.query({ query: (_a = ["{ foo }"], _a.raw = ["{ foo }"], gql(_a)) })
                .then(function (result) {
        ...
        it('transforms watchQuery() results', function () {
            response = { data: { foo: 123 } };
            var observable = client.watchQuery({ query: (_a = ["{ foo }"], _a.raw = ["{ foo }"], gql(_a)) });
            return observableToPromise({ observable: observable }, function (result) {
                assert.deepEqual(result.data, { foo: 123, transformCount: 1 });
                response = { data: { foo: 456 } };
                observable.refetch();
            }, function (result) { return assert.deepEqual(result.data, { foo: 456, transformCount: 2 }); });
            var _a;
        });
        it('transforms watchQuery() currentResult()', function () {
            response = { data: { foo: 123 } };
            var observable = client.watchQuery({ query: (_a = ["{ foo }"], _a.raw = ["{ foo }"], gql(_a)) });
            return observableToPromise({ observable: observable }, function () {
        ...
        it('does not transform identical watchQuery() results', function () {
            ...
            var succeed;
            return Promise.race([
                observableToPromise({ observable: observable, shouldResolve: false }, function (result) {
                    assert.deepEqual(result.data, { foo: 123, transformCount: 1 });
        ...
        it('transforms mutate() results', function () {
            response = { data: { foo: 123 } };
            return client.mutate({ mutation: (_a = ["mutation makeChanges { foo }"], _a.raw = ["mutation makeChanges { foo }"], gql(_a)) })
                .then(function (result) {
                assert.deepEqual(result.data, { foo: 123, transformCount: 1 });
            });
            var _a;
