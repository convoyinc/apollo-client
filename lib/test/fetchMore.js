...
describe('fetchMore on an observable query', function () {
    ...
    it('will set the network status to `fetchMore`', function (done) {
        networkInterface = mockNetworkInterface({ request: { query: query, variables: variables }, result: result, delay: 5 }, {
            request: { query: query, variables: variablesMore },
            result: resultMore,
            delay: 5,
        });
        client = new ApolloClient({
    ...
    it('will get an error from `fetchMore` if thrown', function (done) {
        ...
        observable.subscribe({
            ...
            complete: function () {
                return done(new Error('`complete` called when it wasn’t supposed to be.'));
            },
        });
    });
    var _a, _b;
});
describe('fetchMore on an observable query with connection', function () {
    ...
    it('fetchMore with connection results merging', function () {
        latestResult = null;
        return setup({
            request: {
                query: query,
