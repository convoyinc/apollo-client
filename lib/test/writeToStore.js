...
describe('writing to the store', function () {
    ...
    it('properly normalizes a nested object with an ID', function () {
        ...
        chai_1.assert.deepEqual(writeToStore_1.writeFragmentToStore({
            ...
            _b[result.id] = _.assign({}, _.assign({}, _.omit(result, 'nestedObj')), {
                nestedObj: {
                    type: 'id',
    ...
    it('properly normalizes a nested object without an ID', function () {
        ...
        chai_1.assert.deepEqual(writeToStore_1.writeFragmentToStore({
            ...
            _b["$" + result.id + ".nestedObj"] = result.nestedObj,
            _b
        ));
        var _a, _b;
    });
    it('properly normalizes a nested object with arguments but without an ID', function () {
        ...
        chai_1.assert.deepEqual(writeToStore_1.writeFragmentToStore({
            ...
            _b["$" + result.id + ".nestedObj({\"arg\":\"val\"})"] = result.nestedObj,
            _b
        ));
        var _a, _b;
    });
    it('properly normalizes a nested array with IDs', function () {
        ...
        chai_1.assert.deepEqual(writeToStore_1.writeFragmentToStore({
            ...
            _b[result.id] = _.assign({}, _.assign({}, _.omit(result, 'nestedArray')), {
    ...
    it('properly normalizes a nested array without IDs', function () {
        ...
        chai_1.assert.deepEqual(normalized, (_b = {},
            ...
            _b[result.id + ".nestedArray.0"] = result.nestedArray[0],
            _b[result.id + ".nestedArray.1"] = result.nestedArray[1],
            _b
        ));
        var _a, _b;
    });
    ...
    it('properly normalizes an array of non-objects', function () {
        ...
        chai_1.assert.deepEqual(normalized, (_b = {},
            _b[result.id] = _.assign({}, _.assign({}, _.omit(result, 'simpleArray')), {
                simpleArray: {
                    type: 'json',
                    'json': [
    ...
    it('throw an error if a variable is not provided', function () {
        var testData = [
            ...
            {
                mutation: (_b = ["mutation mut($v: ID) { mut(enum: OK) { id } }"], _b.raw = ["mutation mut($v: ID) { mut(enum: OK) { id } }"], graphql_tag_1.default(_b)),
                variables: { v: '1' },
    ...
    it('does not change object references if the value is the same', function () {
        ...
        var newStore = writeToStore_1.writeFragmentToStore({
            fragment: fragment,
            result: _.cloneDeep(result),
