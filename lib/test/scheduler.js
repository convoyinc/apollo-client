...
describe('QueryScheduler', function () {
    ...
    it('should correctly start polling queries', function (done) {
        ...
        var timesFired = 0;
        var queryId = scheduler.startPollingQuery(queryOptions, 'fake-id', true, function (queryStoreValue) {
            timesFired += 1;
        });
        setTimeout(function () {
    ...
    it('should keep track of in flight queries', function (done) {
        var query = (_a = ["\n      query {\n        fortuneCookie\n      }"], _a.raw = ["\n      query {\n        fortuneCookie\n      }"], graphql_tag_1.default(_a));
        var data = {
            'fortuneCookie': 'lol',
        };
        ...
        chai_1.assert.equal(Object.keys(scheduler.inFlightQueries).length, 1);
        setTimeout(function () {
            chai_1.assert.equal(Object.keys(scheduler.inFlightQueries).length, 1);
            chai_1.assert.deepEqual(scheduler.inFlightQueries[0], queryOptions);
            subscription.unsubscribe();
            done();
        }, 100);
        var _a;
