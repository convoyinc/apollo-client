...
describe('QueryScheduler', function () {
    ...
    it('should handle network errors on polling queries correctly', function (done) {
        ...
        var subscription = observableQuery.subscribe({
            ...
            error: function (errorVal) {
                assert(errorVal);
                var queryId = scheduler.intervalQueries[queryOptions.pollInterval][0];
                assert.isFalse(scheduler.checkInFlight(queryId), 'Should be able to poll after an error');
                subscription.unsubscribe();
                done();
            },
        });
    ...
    it('should remove queries from the interval list correctly', function (done) {
        ...
        var timesFired = 0;
        var observable = scheduler.registerPollingQuery({
            query: query,
            pollInterval: 10,
        });
